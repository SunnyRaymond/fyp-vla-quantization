#!/usr/bin/env python3
"""Read-only verifier for the frozen Rank Student V2 Stage A.

Only JSON/JSONL artifacts are read.  Stable rankings and recall are recomputed
from the recorded arrays.  This verifier does not load models, run inference,
access CUDA, use hashes, or place raw candidate arrays in its result summary.
"""
...
CONTRACT_NAME = "RANK_STUDENT_V2_STAGE_A_FREEZE.json"
SCHEMA = "dino-wm-shared-prefix-cache.rank-student-v2-stage-a-freeze"
TEACHER_SCHEMA = "dino-wm-shared-prefix-cache.rank-student-v2-teacher-record"
EVAL_SCHEMA = "dino-wm-shared-prefix-cache.rank-student-v2-evaluation-record"
TRAINING_SCHEMA = "dino-wm-shared-prefix-cache.rank-student-v2-training-summary"
SELECTION_SCHEMA = "dino-wm-shared-prefix-cache.rank-student-v2-selection"

TRAIN_IDS = tuple(f"wall_v2_case_{index:02d}" for index in range(20, 84))
CALIBRATION_IDS = tuple(f"wall_v2_case_{index:02d}" for index in range(84, 92))
HELDOUT_IDS = tuple(f"wall_v2_case_{index:02d}" for index in range(92, 100))
...
SUMMARY_ARRAY_KEYS = {
    "candidate_actions",
    "J_full",
    "student_context_scores",
    "student_action_only_scores",
    "full_top30",
    "context_top_M",
    "action_only_top_M",
    "recalls",
...
def summary_array_key_paths(value: Any, path: str = "summary") -> list[str]:
    """Find raw candidate-level arrays that must stay in JSONL records."""

    paths: list[str] = []
    if isinstance(value, dict):
        for key, child in value.items():
            child_path = f"{path}.{key}"
            if key in SUMMARY_ARRAY_KEYS:
                paths.append(child_path)
            else:
                paths.extend(summary_array_key_paths(child, child_path))
    elif isinstance(value, list):
        for index, child in enumerate(value):
            paths.extend(summary_array_key_paths(child, f"{path}[{index}]"))
    return paths
...
def validate_contract(contract: Any, errors: dict[str, list[str]]) -> None:
    if not isinstance(contract, dict):
        ...
        return
    for key, expected in (("schema", SCHEMA), ("schema_version", 2), ("status", "frozen")):
        if contract.get(key) != expected:
            add_error(errors, "contract", f"contract.{key}", f"must equal {expected!r}")
    prior = contract.get("prior_failed_recipe", {})
    if prior.get("job_id") != "23895140.pbs101" or prior.get("status") != "honest_no_go":
        add_error(errors, "contract", "contract.prior_failed_recipe", "must preserve V1 job 23895140.pbs101 as honest_no_go")
    splits = contract.get("splits")
    if not isinstance(splits, dict):
        add_error(errors, "contract", "contract.splits", "must be an object")
    else:
        for name, expected in (("train", TRAIN_IDS), ("calibration", CALIBRATION_IDS), ("heldout", HELDOUT_IDS)):
        ...
        if splits.get("disjoint_required") is not True or splits.get("v1_observations_excluded") is not True:
            add_error(errors, "contract", "contract.splits", "must require disjoint V2 splits with V1 observations excluded")
    settings = contract.get("scope", {}).get("fixed_settings", {})
    for key, expected in (("candidate_count", K), ("horizon", HORIZON), ("topk", TOPK), ("cem_opt_steps", OPT_STEPS)):
    ...
    architecture = contract.get("architecture_freeze", {})
    if architecture.get("context_aware_student") != CONTEXT_ARCHITECTURE:
        add_error(errors, "contract", "contract.architecture_freeze.context_aware_student", "does not match frozen V2 cross-attention identity")
    action_only = dict(ACTION_ONLY_ARCHITECTURE)
    action_only["gate_required"] = False
    action_only["report_required"] = True
    if architecture.get("action_only_control") != action_only:
        add_error(errors, "contract", "contract.architecture_freeze.action_only_control", "does not match frozen V2 action-only identity")
    selection = contract.get("selection_gate", {})
    if selection.get("candidate_M_grid") != list(M_GRID) or selection.get("selection_source") != "calibration_only":
        add_error(errors, "contract", "contract.selection_gate", "must freeze M grid and calibration-only source")
    if selection.get("no_heldout_tuning") is not True or selection.get("gate_origin") != "new_v2_gate_not_retuned_from_v1":
        add_error(errors, "contract", "contract.selection_gate", "must forbid heldout tuning and identify a new V2 gate")
...
def validate_common_record(
    ...
    if not isinstance(record, dict):
        ...
        return None
    for key, expected in (("schema", expected_schema), ("schema_version", 2), ("split", expected_split)):
    ...
    if observation_id not in expected_ids:
        add_error(errors, "split_identity", f"{where}.observation_id", "is not in the frozen V2 split")
    ...
    for round_index in ROUNDS:
        ...
        actions = require(item, "candidate_actions", round_where, errors, "schema_completeness")
        if not valid_candidate_actions(actions):
        ...
        if round_index > 0:
            if not same_numeric_array(item.get("input_mu"), previous_mu):
                add_error(errors, "teacher_only_cem_update", round_where, "input_mu does not continue preceding output_mu")
            if not same_numeric_array(item.get("input_sigma"), previous_sigma):
                add_error(errors, "teacher_only_cem_update", round_where, "input_sigma does not continue preceding output_sigma")
...
def validate_records(
    records: list[Any], expected_schema: str, expected_split: str,
    expected_ids: tuple[str, ...], errors: dict[str, list[str]], teacher_only: bool,
) -> None:
    if len(records) != len(expected_ids):
        add_error(errors, "split_identity", expected_split, f"must contain exactly {len(expected_ids)} records")
    seen: list[str] = []
    for number, record in enumerate(records, 1):
        validated = validate_common_record(record, expected_schema, expected_split, expected_ids, f"{expected_split} line {number}", errors, teacher_only)
        if isinstance(validated, dict):
            observation_id = validated.get("observation_id")
            if observation_id in seen:
                add_error(errors, "split_identity", f"{expected_split} line {number}", "observation_id is duplicated")
            elif observation_id in expected_ids:
                seen.append(observation_id)
    if tuple(seen) != expected_ids:
        add_error(errors, "split_identity", expected_split, f"observation IDs must be exactly {list(expected_ids)!r}")


def call_rows(records: list[Any]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for record in records:
        if isinstance(record, dict) and isinstance(record.get("rounds"), list):
            rows.extend(item for item in record["rounds"] if isinstance(item, dict))
    return rows
...
def recall_metrics(records: list[Any], candidate_m: int) -> dict[str, Any]:
    ...
    for item in call_rows(records):
        ...
        context_values.append(len(full_top.intersection(context_top)) / TOPK)
        action_values.append(len(full_top.intersection(action_top)) / TOPK)

    def compact(values: list[float]) -> dict[str, Any]:
        if not values:
            return {"call_count": 0, "median_recall": None, "min_recall": None, "mean_recall": None}
    ...
    return {"context_aware": compact(context_values), "action_only": compact(action_values)}


def metrics_match(recorded: Any, expected: dict[str, Any]) -> bool:
    if not isinstance(recorded, dict) or "recalls" in recorded:
        return False
    for key in ("call_count", "median_recall", "min_recall", "mean_recall"):
...
def validate_training_summary(summary: Any, errors: dict[str, list[str]]) -> None:
    if not isinstance(summary, dict):
        ...
        return
    for key, expected in (("schema", TRAINING_SCHEMA), ("schema_version", 2), ("training_status", "completed")):
        if summary.get(key) != expected:
            add_error(errors, "training_provenance", f"student_training_summary.{key}", f"must equal {expected!r}")
    if summary.get("teacher_data_file") != "teacher_data.jsonl" or summary.get("train_observation_ids") != list(TRAIN_IDS):
        add_error(errors, "training_provenance", "student_training_summary", "must identify the frozen V2 teacher-data file and train IDs")
    if summary.get("teacher_data_observation_count") != len(TRAIN_IDS) or summary.get("teacher_data_round_count") != len(TRAIN_IDS) * len(ROUNDS):
        add_error(errors, "training_provenance", "student_training_summary", "teacher-data counts must be 64 observations and 640 rounds")
    ...
    if summary.get("calibration_used_for_training") is not False or summary.get("heldout_used_for_training") is not False:
        add_error(errors, "no_heldout_tuning", "student_training_summary", "calibration and heldout training flags must be false")
    if summary.get("calibration_and_heldout_in_training") is not False:
        add_error(errors, "no_heldout_tuning", "student_training_summary.calibration_and_heldout_in_training", "must be false")
    for path in summary_array_key_paths(summary):
        add_error(errors, "summary_compactness", path, "raw candidate-level arrays must remain in JSONL records, not training summary")
    context = summary.get("context_aware_student")
    if not isinstance(context, dict) or context.get("status") != "trained" or context.get("architecture") != CONTEXT_ARCHITECTURE:
        add_error(errors, "architecture_identity", "student_training_summary.context_aware_student", "must report the frozen V2 cross-attention architecture")
    control = summary.get("action_only_control")
    if not isinstance(control, dict) or control.get("reported") is not True or control.get("architecture") != ACTION_ONLY_ARCHITECTURE:
        add_error(errors, "context_vs_action_only", "student_training_summary.action_only_control", "must report the frozen action-only architecture")


def validate_selection(selection: Any, calibration_records: list[Any], errors: dict[str, list[str]]) -> int | None:
    if not isinstance(selection, dict):
        add_error(errors, "selection_provenance", "selection", "must be a JSON object")
        return None
    for key, expected in (("schema", SELECTION_SCHEMA), ("schema_version", 2)):
        if selection.get(key) != expected:
            add_error(errors, "selection_provenance", f"selection.{key}", f"must equal {expected!r}")
    if selection.get("candidate_M_grid") != list(M_GRID) or selection.get("selection_rule") != "smallest_feasible_M_calibration_only":
        add_error(errors, "selection_provenance", "selection", "must identify the frozen V2 M grid and rule")
    ...
    if selection.get("calibration_observation_ids") != list(CALIBRATION_IDS) or selection.get("heldout_observation_ids") != list(HELDOUT_IDS):
        add_error(errors, "selection_provenance", "selection", "must identify the frozen V2 calibration and heldout IDs")
    ...
    expected = {str(candidate_m): recall_metrics(calibration_records, candidate_m) for candidate_m in M_GRID}
    recorded_metrics = selection.get("calibration_metrics")
    if not isinstance(recorded_metrics, dict):
        add_error(errors, "selection_provenance", "selection.calibration_metrics", "must record compact metrics for every M")
    else:
        for candidate_m in M_GRID:
            ...
            if not isinstance(recorded, dict):
                ...
                continue
            for model_name in ("context_aware", "action_only"):
                if not metrics_match(recorded.get(model_name), expected[str(candidate_m)][model_name]):
                    add_error(errors, "selection_provenance", f"selection.calibration_metrics.{candidate_m}.{model_name}", "does not match recomputed compact metrics")
    for path in summary_array_key_paths(selection):
        add_error(errors, "summary_compactness", path, "raw candidate-level arrays must not be stored in selection artifact")

    feasible: list[int] = []
    for candidate_m in M_GRID:
        context = expected[str(candidate_m)]["context_aware"]
        if context["call_count"] == len(CALIBRATION_IDS) * len(ROUNDS) and context["median_recall"] == 1.0 and context["min_recall"] >= 29 / 30:
...
def verify(contract: Any, teacher_records: list[Any], training_summary: Any, calibration_records: list[Any], heldout_records: list[Any], selection: Any) -> dict[str, Any]:
    errors: dict[str, list[str]] = {}
    ...
    if selected_m is not None:
        ...
        heldout_pass = selected_m <= 180 and context["call_count"] == len(HELDOUT_IDS) * len(ROUNDS) and context["median_recall"] == 1.0 and context["min_recall"] >= 29 / 30
        if not heldout_pass:
            add_error(errors, "heldout_gate", "heldout", "selected fixed M does not meet V2 context-aware median/min gate")
    else:
        add_error(errors, "heldout_gate", "heldout", "cannot evaluate heldout gate without a calibration-selected M")

    gates = {
        "contract": not errors.get("contract"),
        "split_identity": not errors.get("split_identity"),
        "schema_completeness": not errors.get("schema_completeness"),
        "train_only_student_training": not errors.get("training_provenance"),
        "architecture_identity": not errors.get("architecture_identity"),
        "summary_compactness": not errors.get("summary_compactness"),
        "selection_provenance": not errors.get("selection_provenance"),
        "no_heldout_tuning": not errors.get("no_heldout_tuning"),
        "teacher_only_cem_update": not errors.get("teacher_only_cem_update"),
        "context_vs_action_only_recorded": not errors.get("context_vs_action_only"),
        "heldout_context_aware_gate": heldout_pass and not errors.get("heldout_gate"),
    }
    verdict = "PASS" if all(gates.values()) else "FAIL"
    compact_calibration = {str(candidate_m): recall_metrics(calibration_records, candidate_m) for candidate_m in M_GRID}
    return {
        "verdict": verdict,
        "stage": "stage_a_v2_teacher_student_calibration_heldout_offline",
        "decision": {
            "selected_M": selected_m,
            "stage_a_offline_screen": verdict,
            "later_chained_stage_b_authorized": verdict == "PASS",
            "prior_v1_job": "23895140.pbs101",
            "prior_v1_status": "honest_no_go",
...
def main() -> int:
    ...
    try:
        result = verify(
            ...
            load_jsonl(paths["teacher_data"]),
            load_json(paths["training_summary"]),
            load_jsonl(paths["calibration"]),
            load_jsonl(paths["heldout"]),
            load_json(paths["selection"]),
        )
