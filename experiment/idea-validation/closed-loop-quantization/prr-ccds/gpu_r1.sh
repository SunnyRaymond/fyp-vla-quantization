...
#SBATCH --job-name=prr_r1
#SBATCH --output=artifacts/r1_%j.log
set -euo pipefail
TOP="$HOME/prr_ccds"
...
cp "$TOP/control/manifest_r1.json" "$OUT/manifest.json"
cp "$TOP/artifacts/64705/engineering_summary.json" "$OUT/engineering_summary.json"
...
timeout --signal=TERM --kill-after=15s 5m "$OLD/venv/bin/python" "$OUT/prr_records.py" --stage r1 --root "$OLD/modelroot" --manifest "$OUT/manifest.json" --output "$OUT/records" --helper-dir "$OLD/control"
timeout --signal=TERM --kill-after=15s 39m "$OLD/venv/bin/python" "$OUT/prr_runner.py" --stage formal --root "$OLD/modelroot" --manifest "$OUT/records/manifest.json" --output "$OUT"
