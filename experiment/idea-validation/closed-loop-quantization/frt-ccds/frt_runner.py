...
def _core():
    try:
        from frt_core import (  # type: ignore
            SourceHistoryAdapter,
            W4Spec,
            action_mask_like,
            atomic_json,
            atomic_npz,
...
def _load_smoke(manifest: Mapping[str, Any]) -> Any:
    ...
    candidates.extend([
        HERE.parent.parent.parent / "world-model-quantization" / "experiments" / "dino-wm-wall" / "smoke_runner.py",
        HERE / "smoke_runner.py",
        Path.cwd() / "smoke_runner.py",
    ])
    try:
        import smoke_runner
...
def _identity(manifest: Mapping[str, Any], manifest_path: Path, run_id: str, stage: str, identity: Mapping[str, Any], groups: Sequence[Mapping[str, Any]], allocation: Mapping[str, Any], gpu: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema": "frt-ccds-progress-v1",
        "run_id": run_id,
        "stage": stage,
        "manifest_sha256": _path_hash(manifest_path),
        "runtime_identity": _stable_runtime(identity),
        "target_groups": [
            {"group_id": group.get("group_id"), "linear": group.get("linear"), "numel": group.get("numel")}
            for group in groups
        ],
        "allocation": {key: allocation.get(key) for key in ("scheduler", "job_id", "hostname", "nodelist")},
        "gpu": gpu,
    }
...
def _build_bank(
    ...
    with torch.no_grad():
        for row in q0_rows:
            ...
            if not torch.allclose(q0_history, expected, rtol=1e-5, atol=1e-6):
    ...
    if wz is None:
        wz = core.estimate_wz(fp_current, floor=1e-3).detach().cpu()
    ...
    provenance_keys = (
        "episode_id", "record_index", "local_index", "dataset_index", "subset_index",
        "original_dataset_index", "window_start", "env_seed", "cem_seed",
    )
    metadata = []
    for row in rows:
    ...
    return {
        ...
        "q0_meta": q0_meta,
        "history": histories,
        "x_history": x_histories,
        "action": actions,
        "next_action": next_actions,
        "fp_current": fp_current,
        "q0_current": q0_current,
        "q0_transport": q0_transport,
        "delta": delta,
        "fp_transport": fp_transport,
        "random_delta": random_delta,
        "random_transport": random_transport,
        "wz": wz,
        "action_mask": adapter.action_mask(delta[:1])[0, 0].detach().cpu(),
        "active_mask": (~adapter.action_mask(delta[:1])[0, 0]).detach().cpu(),
    }
...
def _rounding_config(manifest: Mapping[str, Any]) -> dict[str, float]:
    ...
    if not isinstance(fit, Mapping):
        fit = {}
    return {
        "temperature_start": float(fit.get("temperature_start", 2.0)),
        "temperature_end": float(fit.get("temperature_end", 0.1)),
        "regularization_start": float(fit.get("rounding_regularization_start", fit.get("rounding_regularization", 0.01))),
        "regularization_end": float(fit.get("rounding_regularization_end", fit.get("rounding_regularization", 0.1))),
    }
...
def _fit_method(
    ...
    output: Path,
    progress_path: Path,
    progress_state: MutableMapping[str, Any],
) -> dict[str, Any]:
    core = _core()
    torch = runtime["torch"]
    model = runtime["model"]
    ...
    lr = float(optimizer_cfg.get("lr", fit_cfg.get("lr", manifest.get("lr", 1e-2))))
    ...
    for step in range(steps):
        ...
        with torch.enable_grad():
            ...
            losses = core.frt_loss(
                clean_prediction,
                device_bank["fp_current"].index_select(0, idx),
                transport_prediction,
                device_bank[target_key].index_select(0, idx),
                wz,
                lambda_transport if method != "clean" else 0.0,
            )
        ...
        if step == 0 or (step + 1) == steps or (step + 1) % max(1, min(25, steps // 5 or 1)) == 0:
            fit_rows.append({
                "step": step + 1,
                "clean": float(losses["clean"].detach().item()),
                "transport": float(losses["transport"].detach().item()),
                "rounding_regularizer": float(regularizer.detach().item()),
    ...
    result = {
        ...
        "lambda_transport_source": "fit.loss" if lambda_value is not None else "clean_only_zero",
        "rounding": {**rounding, "final_temperature": temperature, "final_regularization_weight": regularization_weight},
        "elapsed_seconds": elapsed,
        "checkpoint": str(checkpoint_path.resolve()),
        "hard_ledger": hard_ledger,
        "fit_trace": fit_rows,
    }
    return result
...
def _evaluate_model(
    ...
    with torch.no_grad():
        ...
        metrics = {
            "clean_mse": _metric_row(core.weighted_mse(clean, data["fp_current"], wz)),
            "frozen_q0_transport_mse": _metric_row(core.weighted_mse(frozen_transport, data["fp_transport"], wz)),
            "requested_transport_mse": _metric_row(core.weighted_mse(requested_transport, data[target_key], wz)),
            "mean_q0_delta_norm": _metric_row(data["delta"].reshape(data["delta"].shape[0], -1).norm(dim=1).mean()),
        }
        theta_current = clean
...
def _source_bridge_gate(rows: Sequence[Mapping[str, Any]], adapter: Any, torch: Any, device: Any) -> dict[str, Any]:
    for row in rows:
        ...
        close = bool(torch.allclose(expected, observed, rtol=1e-5, atol=1e-6))
...
def _timed_backward(
    ...
    optimizer = torch.optim.Adam(params, lr=float(opt_cfg.get("lr", fit_cfg.get("lr", 1e-2))))
    direction_key, target_key = _method_direction(bank, method)
    with torch.enable_grad():
        for handle in handles:
    ...
    return {
        ...
        "seed": int(seed),
        "elapsed_seconds": elapsed,
        "seconds_per_fit_step": elapsed,
        "peak_vram_allocated_gib": peak_allocated,
        "peak_vram_reserved_gib": peak_reserved,
        "soft_clean_loss": float(losses["clean"].detach().item()),
        "soft_transport_loss": float(losses["transport"].detach().item()),
        "soft_total_loss": float(total.detach().item()),
        "rounding_regularizer": float(reg.detach().item()),
        "temperature": 2.0,
        "finite": core.tensor_finite(total),
    }


def _hard_reload_gate(
    runtime: Mapping[str, Any], modules: Sequence[tuple[str, Any]], snapshot: Mapping[str, Any], bank: Mapping[str, Any], adapter: Any, output: Path
) -> dict[str, Any]:
    core = _core()
    torch = runtime["torch"]
    model = runtime["model"]
...
def _stage_a(
    ...
    if not source_rows:
        result = {
            "schema": "frt-stage-a-v1", "status": "blocked", "engineering_pass": False,
            "reason": "manifest has no stage_a records; collector must provide full history/action and replay evidence",
    ...
    with torch.no_grad():
        ...
        delta_zero_gate = {"passed": float(zero_transport.abs().max().item()) <= 1e-7, "max_abs_transport": float(zero_transport.abs().max().item())}
    ...
    safe_fit_steps = max(1, int(safe_budget / max(max_step, 1e-9)))
    ...
    summary = {
        "schema": "frt-stage-a-v1",
        "status": "complete",
        "engineering_pass": bool(source_gate["passed"] and physical_gate["passed"] and delta_zero_gate["passed"] and fp_null_gate["passed"] and invariant_gate["passed"] and hard_gate["passed"] and learned_gate["passed"] and all(row["finite"] for row in timings.values())),
        "allocation": dict(allocation),
        "gpu": gpu,
        "runtime_identity": runtime_identity,
        "target_groups": _target_modules_identity(groups),
        "gates": {"source_rollout_agreement": source_gate, "physical_replay": physical_gate, "delta_zero": delta_zero_gate, "fp_transport_null": fp_null_gate, "history_action_invariants": invariant_gate, "hard_materialize_reload": hard_gate},
        "seconds_per_fit_step_by_method": {method: row["seconds_per_fit_step"] for method, row in timings.items()},
        "backward_timing": timings,
        "peak_memory_by_method": {method: {"allocated_gib": row["peak_vram_allocated_gib"], "reserved_gib": row["peak_vram_reserved_gib"]} for method, row in timings.items()},
        "safe_1000_step_estimate_seconds_by_method": {method: float(row["seconds_per_fit_step"] * 1000 * 1.25) for method, row in timings.items()},
        "safe_fit_steps": safe_fit_steps,
        "calibration_freeze": {"mean_L_clean_Q0": q0_clean_mse, "mean_L_transport_Q0": q0_transport_mse, "q0_clean_mse_wz": q0_clean_mse, "q0_transport_mse_wz": q0_transport_mse, "lambda_raw": lambda_raw, "lambda_T": lambda_t, "lambda_clip": [0.25, 4.0], "lambda_clamped": bool(lambda_t != lambda_raw), "scope_warning": lambda_warning},
        "q0_probe": {"records": len(source_rows), "timing_records": probe_count, "mean_delta_norm": float(data["delta"].reshape(data["delta"].shape[0], -1).norm(dim=1).mean().item())},
        "execution": "fake_quant_emulation_only",
    }
...
def _stage_b(
    ...
    core = _core()
    a_summary_path = _resolve(args.a_summary, manifest_path.parent) if args.a_summary else output.parent / "stage_a_summary.json"
    if not a_summary_path.is_file():
        raise FileNotFoundError(f"Stage A summary is required before B: {a_summary_path}")
    a_summary = _json(a_summary_path)
    ...
    model.eval()
    adapter = core.SourceHistoryAdapter(model)
    cal_raw, dev_raw = _split_sources(manifest, manifest_path.parent)
    cal_raw = _validate_split_rows(cal_raw, "cal", require_fresh=True)
    dev_raw = _validate_split_rows(dev_raw, "dev", require_fresh=True)
    _validate_split_pair(cal_raw, dev_raw)
    for split, rows in (("cal", cal_raw), ("dev", dev_raw)):
        expected_indices = _declared_indices(manifest, split)
        if expected_indices is not None:
            actual_indices = {int(row["dataset_index"]) for row in rows}
            if actual_indices != expected_indices:
                raise ValueError(f"{split} dataset indices disagree with manifest: {sorted(actual_indices)} != {sorted(expected_indices)}")
    device = next(model.parameters()).device
    cal_rows = _expand_batched_rows(cal_raw, adapter, torch, device, "cal")
    dev_rows = _expand_batched_rows(dev_raw, adapter, torch, device, "dev")
    if len({str(row["episode_id"]) for row in cal_rows}) != 6 or len({str(row["episode_id"]) for row in dev_rows}) != 6:
        raise ValueError("CAL/DEV must each have exactly six episode identities after expansion")
    snapshot = core.snapshot_modules(modules, cpu=True)
    cal_bank = _build_bank(runtime, modules, snapshot, cal_rows, adapter, "cal", int(manifest.get("random_seed", 910001)))
    cal_wz = cal_bank["wz"].detach().cpu()
    dev_bank = _build_bank(runtime, modules, snapshot, dev_rows, adapter, "dev", int(manifest.get("random_seed", 910001)) + 1, wz=cal_wz)
    output.mkdir(parents=True, exist_ok=True)
    _save_bank(output / "bank_cal.npz", cal_bank, core)
    _save_bank(output / "bank_dev.npz", dev_bank, core)
    fit_cfg = manifest.get("fit", {}) if isinstance(manifest.get("fit", {}), Mapping) else {}
    methods_value = fit_cfg.get("methods", manifest.get("methods", list(METHODS)))
    methods = list(methods_value) if isinstance(methods_value, Sequence) and not isinstance(methods_value, (str, bytes)) else list(METHODS)
    aliases = {"random": "random_same_norm", "random_direction": "random_same_norm", "FRT": "frt", "clean_only": "clean"}
    methods = [aliases.get(str(method), str(method)) for method in methods]
    if set(methods) - set(METHODS):
        raise ValueError(f"unsupported methods: {methods}")
    seeds_value = fit_cfg.get("seeds", manifest.get("seeds"))
    if not isinstance(seeds_value, Sequence) or isinstance(seeds_value, (str, bytes)) or len(seeds_value) != 3:
        raise ValueError("Stage B requires exactly three explicit fitting seeds")
    seeds = [int(value) for value in seeds_value]
    fit_steps = fit_cfg.get("fit_updates", fit_cfg.get("steps", manifest.get("fit_steps")))
    if fit_steps is None:
        fit_steps = a_summary.get("safe_fit_steps")
    if fit_steps is None or int(fit_steps) <= 0:
        raise ValueError("bounded fit updates are missing; freeze them from Stage A timing")
    fit_cfg = dict(fit_cfg)
    fit_cfg["fit_updates"] = int(fit_steps)
    freeze = a_summary.get("calibration_freeze", {})
    lambda_t = freeze.get("lambda_T") if isinstance(freeze, Mapping) else None
    if lambda_t is None:
        raise ValueError("Stage A did not provide frozen calibration lambda_T")
    loss_cfg = dict(fit_cfg.get("loss", {})) if isinstance(fit_cfg.get("loss", {}), Mapping) else {}
    loss_cfg["lambda_T"] = float(lambda_t)
    fit_cfg["loss"] = loss_cfg
    manifest = dict(manifest)
    manifest["fit"] = fit_cfg
    results = []
    data_identity = {"runtime_identity": runtime_identity, "target_groups": _target_modules_identity(groups), "cal_episodes": sorted({str(row["episode_id"]) for row in cal_rows}), "dev_episodes": sorted({str(row["episode_id"]) for row in dev_rows}), "fit_updates": int(fit_steps), "seeds": seeds, "methods": methods, "lambda_T": float(lambda_t)}
    for method in methods:
        for seed in seeds:
            key = f"{method}:{seed}"
            if key in progress_state.get("completed", []):
                result_path = output / "methods" / f"{method}_seed_{seed}.json"
                if result_path.is_file():
                    results.append(_json(result_path))
                    continue
            core.set_seed(seed)
            result = _fit_method(runtime, modules, snapshot, cal_bank, adapter, method, seed, manifest, output, progress_path, progress_state)
            result["cal"] = _evaluate_model(runtime, modules, snapshot, cal_bank, adapter, cal_wz.to(device=device), method)
            result["dev"] = _evaluate_model(runtime, modules, snapshot, dev_bank, adapter, cal_wz.to(device=device), method)
            result["identity"] = data_identity
            result_path = output / "methods" / f"{method}_seed_{seed}.json"
            ...
            _progress(progress_path, progress_state, completed=completed, current_method=None, current_step=None)
            core.restore_modules(modules, snapshot)
    summary = {
        "schema": "frt-stage-b-v1", "status": "complete", "engineering_pass": True,
        "allocation": dict(allocation), "gpu": gpu, "runtime_identity": runtime_identity,
        "target_groups": _target_modules_identity(groups), "data_identity": data_identity,
        "cal_records": len(cal_rows), "dev_records": len(dev_rows),
        "cal_episodes": sorted({str(row["episode_id"]) for row in cal_rows}),
        "dev_episodes": sorted({str(row["episode_id"]) for row in dev_rows}),
        "fit_updates": int(fit_steps), "seeds": seeds, "methods": methods, "lambda_T": float(lambda_t),
        "wz": cal_wz.tolist(), "results": results,
        "test_opened": False, "execution": "fake_quant_emulation_only",
    }
    core.atomic_json(output / "stage_b_summary.json", summary)
    core.atomic_json(output / "summary.json", summary)
    _progress(progress_path, progress_state, status="complete", result="stage_b_summary.json")
    return summary
