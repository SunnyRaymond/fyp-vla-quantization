...
def _runtime(root: Path, device: str | None = None):
    ...
    # This is the same loader used by experiment/reproduction/dino-wm-wall/evaluate_wall.py.
    _set_seed(100000)
...
def _group_record(family: str, index: int, modules: Sequence[Tuple[str, Any]]) -> Dict[str, Any]:
    group_path = (
        f"encoder.base_model.blocks.{index}"
        if family == "encoder"
        else f"predictor.transformer.layers.{index}"
    )
    rows = []
    total = 0
    scale_count = 0
...
def _run_bench(root: Path, out: Path, workload_path: Path, repeats: int, warmup: int,
    ...
    # Warmup is deliberately before the shared barrier and excluded from timing.
