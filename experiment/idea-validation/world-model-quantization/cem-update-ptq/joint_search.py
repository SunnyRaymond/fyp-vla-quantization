"""Bounded CEM-update PTQ search on frozen candidate-pool workloads.

The runner is intentionally a small bridge around the archived Wall
``smoke_runner.py`` helpers.  It performs numerical per-output-channel RTN
fake quantization and therefore makes no native low-bit speed or memory
claim.  Calibration maps are selected from CAL only; DEV is evaluated after
the search and cannot affect map selection.

The program is intended for a real PBS compute allocation.  It does not
submit jobs, download files, or start a model on a login node.
"""
...
CAL_TOL = 1e-10
NMSE_EPS = 1e-12
# Shared CPU-noise stream for the two-step DEV replay.  The historical Wall
# screen uses local-index namespaces for deterministic CEM seeds.
CHAIN_SEED = 810_000
...
def _stable_elite(scores: Sequence[float], candidates: np.ndarray, topk: int, device: Any = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # The actual adapter updates in torch float32.  Keep that dtype for the
    # update statistics (and correction=1/unbiased standard deviation), while
    # retaining float64 for the independently recomputed scalar losses.
    import torch
...
def _pool_metrics(reference: np.ndarray, quantized: np.ndarray, candidates: np.ndarray, topk: int, iteration: int, device: Any = None) -> Dict[str, Any]:
    ...
    # Loss scalars are deliberately accumulated in float64 from the saved
    # float32 CEM statistics for stable 1e-10 calibration comparisons.
...
def _aggregate(rows: Sequence[Mapping[str, Any]], field: str) -> float:
    ...
    if not by_episode:
        return float("nan")
    # pool -> MPC point -> episode; there is one new MPC point per episode in
    # the current workload, but averaging rows by episode preserves the rule.
...
def _benchmark(args: argparse.Namespace, deadline: Deadline, smoke: Any, runtime: Mapping[str, Any], groups: Sequence[Mapping[str, Any]], initial: Mapping[str, int], output: Path, model_load_seconds: float) -> Dict[str, Any]:
    ...
    for repetition in range(repetitions):
        deadline.check()
        # Each repetition deliberately uses a fresh cache so both timings
        # include a complete 18-group apply and all candidate-pool forwards.
    ...
    result = {
        ...
        "helper_source": "same-dir archived smoke/probe/screen/joint helpers from 16177209.pbs101",
...
def _self_test() -> None:
    # Exact tie semantics, correction-1 sigma, stable neighbor count, and the
    # four objective definitions are testable without importing torch/model code.
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--initial", type=Path, required=False, help="frozen old ScoreError allocation JSON")
    ...
    parser.add_argument("--self-test", action="store_true", help="run pure-CPU numerical checks without PBS/model loading")
