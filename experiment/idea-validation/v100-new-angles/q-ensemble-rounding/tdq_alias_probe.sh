...
#SBATCH --job-name=tdq_alias
#SBATCH --output=artifacts/tdq_alias_%j.log
set -euo pipefail
...
test ! -e "${OUT}"
mkdir -p "${OUT}"
cp "${TOP}/control/tdq_alias_probe.py" "${TOP}/control/tdq_alias_probe.sh" "${OUT}/"
...
"${TOP}/smolvla/venv/bin/python" "${OUT}/tdq_alias_probe.py"
