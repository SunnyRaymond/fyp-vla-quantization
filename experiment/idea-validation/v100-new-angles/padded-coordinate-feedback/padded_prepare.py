"""Prepare eight raw samples for the padded-coordinate feedback screen.
...
def _status(out: Path, assetdir: Path, allocation: dict[str, Any], phase: str, state: str = "running", write_asset: bool = True, **extra: Any) -> None:
    value: dict[str, Any] = {
        "schema": "padded-coordinate-preparation-status-v1",
        "state": state,
        "phase": phase,
...
def _main(args: argparse.Namespace, allocation: dict[str, Any], helper: Any) -> None:
    ...
    identity_path = root / "smolvla" / "identity.json"
    ...
    old_manifest = _load_json(root / "smolvla" / "manifest.json", "Flow old manifest")
    if old_manifest.get("schema") != "smolvla-raw-input-manifest-v1":
        raise PreparationError("Flow old manifest schema mismatch")
    excluded_pairs: set[tuple[int, int]] = set()
    for entry in old_manifest.get("samples", []):
        if not isinstance(entry, dict):
            raise PreparationError("Flow old manifest has malformed sample")
        excluded_pairs.add((int(entry["task_index"]), int(entry.get("dataset_episode_index", entry["episode_index"]))))
    if len(excluded_pairs) != 12 or any(sum(task == t for task, _ in excluded_pairs) != 3 for t in range(4)):
        raise PreparationError("Flow old manifest does not contain exactly three episodes per task")
    ...
    _json_dump(out / "selection.json", {
        ...
        "rule": "task_index 0..3; skip Flow manifest three pairs per task; episode_index ascending first two unused; zero-based floor(length/4)",
    ...
    for ordinal, plan in enumerate(sorted(plans, key=lambda item: (item["row"]["task_index"], item["row"]["episode_index"])), 1):
        ...
        for camera, relative in plan["videos"].items():
            ...
            frame, conversion = helper._decode_frame(root / "smolvla" / "source_subset" / relative, video_timestamp, tolerance)
    ...
    manifest = {
        ...
        "selection": {
            ...
            "rule": "skip Flow old manifest three episodes per task; episode_index ascending first two unused; zero-based floor(length/4)",
            "excluded_episode_ids": excluded_ids,
            "excluded_episode_pairs": [{"task_index": t, "episode_index": e} for t, e in sorted(excluded_pairs)],
        },
...
def main() -> None:
    ...
    parser.add_argument("--assetdir", default=str(ROOT_DEFAULT / "padded_feedback"))
