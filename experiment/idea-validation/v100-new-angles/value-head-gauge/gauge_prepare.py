"""Prepare reset-only Cartpole state observations for the value-head gauge.
...
def _status(out: Path, value: dict[str, Any], assetdir: Path | None = None) -> None:
    payload = {
        "schema": "value-head-gauge-preparation-status-v1",
...
def _run(args: argparse.Namespace, allocation: dict[str, Any]) -> None:
    ...
    if assetdir != root and root not in assetdir.parents:
        raise PreparationError(f"gauge asset must remain under preparation root: {assetdir}")
    if assetdir.exists():
        raise PreparationError(f"isolated gauge asset directory already exists: {assetdir}")
    out.mkdir(parents=True, exist_ok=True)
    _status(out, {"state": "running", "phase": "guard_verified"})
    parent_ref = _parent_reference(parent_path)
    runtime = _runtime_identity()
    _status(out, {"state": "running", "phase": "runtime_verified", "runtime": runtime})
...
def main() -> None:
    ...
    parser.add_argument("--assetdir", default=str(TOP_DEFAULT / "value_head_gauge_ready"))
    parser.add_argument("--output", required=True)
    parser.add_argument(
        "--parent-manifest",
        default=os.environ.get("TDQ_PARENT_MANIFEST", ""),
        help="Explicit compatible TDQ parent manifest; no seed-1 default exists.",
    )
