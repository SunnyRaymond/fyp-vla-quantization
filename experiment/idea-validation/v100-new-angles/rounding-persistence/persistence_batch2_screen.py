"""B2-batched matched temporal-coupling screen for SmolVLA expert-only W4 SR snapshots.
...
computes the scientific verdict.  This execution repair batches only the two
fixed noises for one state, then de-batches them back to the frozen raw axes.
The CPU verifier owns the S/cross-term and endpoint metrics.
"""
...
SCHEMA = "rounding-persistence-batch2-screen-v1"
...
EXPECTED_PROTOCOL_SHA256 = "ae3ffb51225848ee951b11b933a6d7a2d642467824378fd5ddd8b2caea079ece"
    ...
    *,
    ...
    def wrapped(*call_args: Any, **call_kwargs: Any) -> Any:
        ...
        if force_x is not None:
            ...
            current_x = fixed
        if not torch.is_tensor(current_x) or tuple(current_x.shape) != (BATCH_SIZE, HORIZON, PADDED_ACTION_DIM):
            raise RuntimeError(f"denoise_step x_t shape is not [2,50,32]: {getattr(current_x, 'shape', None)}")
        cache = bound.arguments["past_key_values"]
        last_cache = cache
        timestep = bound.arguments["timestep"]
        expected_timestep = 1.0 + float(DT) * step
        if not torch.is_tensor(timestep) or tuple(timestep.shape) != (BATCH_SIZE,) or timestep.dtype != torch.float32:
            raise RuntimeError(f"denoise_step timestep shape/dtype drifted from B2 [2]: {getattr(timestep, 'shape', None)}, {getattr(timestep, 'dtype', None)}")
        ...
        x_input_sha256.append([_tensor_bytes_sha256(current_x[index], torch) for index in range(BATCH_SIZE)])
        value = original(*(call_args_list if force_x is not None else call_args), **(call_kwargs if force_x is not None else call_kwargs))
        if not torch.is_tensor(value) or tuple(value.shape) != (BATCH_SIZE, HORIZON, PADDED_ACTION_DIM):
            raise RuntimeError(f"denoise_step velocity shape is not [2,50,32]: {getattr(value, 'shape', None)}")
    ...
    velocity_tensor = torch.stack(velocities, dim=0)
    x_input_tensor = torch.stack(x_inputs, dim=0)
    next_tensor = x_input_tensor[-1] + float(DT) * velocity_tensor[-1]
    x_states = torch.cat([x_input_tensor, next_tensor.unsqueeze(0)], dim=0)
    if tuple(output.shape) != (BATCH_SIZE, HORIZON, PHYSICAL_ACTION_DIM):
        raise RuntimeError(f"physical action output shape is not [2,50,7]: {tuple(output.shape)}")
    ...
    if force_x is None and not torch.allclose(output, x_states[-1, :, :, :PHYSICAL_ACTION_DIM], atol=TOLERANCE, rtol=0):
    ...
    return {
        ...
        "x_input_sha256": np.asarray(x_input_sha256, dtype="<U64").transpose(1, 0),
    }
...
def _manual_fp_action_unwrapped(policy: Any, batch: Mapping[str, Any], noise: Any, torch: Any) -> Any:
    ...
    for step in range(VELOCITY_STEPS):
        ...
        if tuple(value.shape) != (BATCH_SIZE, HORIZON, PADDED_ACTION_DIM):
            raise RuntimeError("manual FP denoise velocity shape mismatch for B2")
...
def _batch_repeat(value: Any, torch: Any) -> Any:
    """Repeat one prepared condition twice without combining states."""
    if torch.is_tensor(value):
        if value.ndim == 0 or int(value.shape[0]) != 1:
            raise RuntimeError(f"prepared tensor is not batch-one: {tuple(value.shape)}")
        return value.expand(BATCH_SIZE, *value.shape[1:]).clone()
    if isinstance(value, Mapping):
        return {key: _batch_repeat(item, torch) for key, item in value.items()}
    if isinstance(value, list):
        if len(value) != 1:
            raise RuntimeError("prepared list metadata is not batch-one")
        return [value[0], value[0]]
    if isinstance(value, tuple):
        if len(value) != 1:
            raise RuntimeError("prepared tuple metadata is not batch-one")
        return (value[0], value[0])
    if isinstance(value, str):
        return value
    raise RuntimeError(f"prepared batch contains unsupported value: {type(value)!r}")
...
def _run(args: argparse.Namespace, allocation: Mapping[str, Any]) -> None:
    ...
    engineering: Dict[str, Any] = {
        ...
        "batch_mode": {
            ...
            "conditions_per_forward": BATCH_SIZE,
            "same_state_only": True,
            "raw_axes": "debatched back to state,noise,arm,time,position,coordinate",
        },
        "parameters": {
            "batch_mode": {
                "batch_size": BATCH_SIZE,
            ...
            "noise_batch_shape": [BATCH_SIZE, HORIZON, PADDED_ACTION_DIM],
            "timestep_batch_shape": [BATCH_SIZE],
            "timestep_source": "LeRobot v0.4.4 sample_actions time_tensor.expand(bsize)",
    ...
    try:
        ...
        # The two fixed noises are the only batch members.  Every state is
        # repeated within its own batch; states are never combined.
        noise_batch = torch.cat(noises, dim=0)
        batch0 = _batch_repeat(batches[0], torch)
        no_op = _run_schedule(helper, policy, batch0, noise_batch, ["FP"] * VELOCITY_STEPS, maps, fp, modules, torch, reload_each_step=False)
        manual_action = _manual_fp_action(policy, batch0, noise_batch, torch)
        ...
        rtn_reuse = _run_schedule(helper, policy, batch0, noise_batch, ["RTN"] * VELOCITY_STEPS, maps, fp, modules, torch, reload_each_step=False)
        rtn_reload = _run_schedule(helper, policy, batch0, noise_batch, ["RTN"] * VELOCITY_STEPS, maps, fp, modules, torch, reload_each_step=True)
        ...
        for state_index, batch in enumerate(batches):
            batch_b2 = _batch_repeat(batch, torch)
            ...
            for arm_index, arm in enumerate(ARM_NAMES):
                if arm.startswith("C"):
                    ...
                    arm_schedule = [arm] * VELOCITY_STEPS
                    reload = False
                result = _run_schedule(helper, policy, batch_b2, noise_batch, arm_schedule, map_by_arm, fp, modules, torch, reload_each_step=reload)
                ...
                if arm == "FP":
                    ...
                    if state_index == 0 and condition_cache_digest != no_op["cache_digest"]:
                ...
                result_x = np.asarray(result["x_states"], dtype=np.float32)
                result_v = np.asarray(result["velocities"], dtype=np.float32)
                result_actions = result["action"].detach().cpu().numpy().astype(np.float32)
                if result_x.shape != (VELOCITY_STEPS + 1, BATCH_SIZE, HORIZON, PADDED_ACTION_DIM) or result_v.shape != (VELOCITY_STEPS, BATCH_SIZE, HORIZON, PADDED_ACTION_DIM) or result_actions.shape != (BATCH_SIZE, HORIZON, PHYSICAL_ACTION_DIM):
                    raise RuntimeError(f"B2 result shape drifted for arm {arm}: x={result_x.shape}, v={result_v.shape}, action={result_actions.shape}")
                raw_x[state_index, :, arm_index] = result_x.transpose(1, 0, 2, 3)
                raw_v[state_index, :, arm_index] = result_v.transpose(1, 0, 2, 3)
                actions[state_index, :, arm_index] = result_actions
                schedule_indices = _labels_to_draw_indices(result["labels"])
                actual_schedule[state_index, :, arm_index] = np.broadcast_to(schedule_indices, (BATCH_SIZE, VELOCITY_STEPS))
                actual_times[state_index, :, arm_index] = np.broadcast_to(result["actual_timesteps"], (BATCH_SIZE, VELOCITY_STEPS))
                if arm == "FP":
                    fp_result = result
                if not np.isfinite(raw_x[state_index, :, arm_index]).all() or not np.isfinite(raw_v[state_index, :, arm_index]).all() or not np.isfinite(actions[state_index, :, arm_index]).all():
                    raise FloatingPointError(f"non-finite raw output at state={state_index}, arm={arm}")
                _restore_fp(modules, fp, torch)
            ...
            fp_x = torch.from_numpy(np.asarray(fp_result["x_states"][:-1], dtype=np.float32)).to(device=device, dtype=torch.float32)
            for draw_index, label in enumerate(DRAW_NAMES):
                common = _run_schedule(helper, policy, batch_b2, noise_batch, [label] * VELOCITY_STEPS, map_by_arm, fp, modules, torch, reload_each_step=False, force_x=fp_x)
                ...
                common_v = np.asarray(common["velocities"], dtype=np.float32)
                common_sha = np.asarray(common["x_input_sha256"], dtype="<U64")
                if common_v.shape != (VELOCITY_STEPS, BATCH_SIZE, HORIZON, PADDED_ACTION_DIM) or common_sha.shape != (BATCH_SIZE, VELOCITY_STEPS):
                    raise RuntimeError(f"B2 common-path shape drifted for {label}: v={common_v.shape}, sha={common_sha.shape}")
                common_q_v[state_index, :, draw_index] = common_v.transpose(1, 0, 2, 3)
                common_x_sha256[state_index, :, draw_index] = common_sha
                _restore_fp(modules, fp, torch)
                if helper._state_subset_digest(policy, expert_state_names, torch) != bypass_digest:
            ...
            # The digest is a shared prefix property for both noises; retain
            # both frozen keys so the original CPU verifier can enumerate the
            # unchanged state/noise axes.
            for noise_index in range(BATCH_SIZE):
            ...
            _write_raw(
                ...
                actual_schedule,
                {
                    ...
                    "common_x_sha256_formula": "sha256(contiguous float32 bytes of each exact per-sample x_t captured immediately before each denoise_step)",
                    "batch_mode": {
                        "batch_size": BATCH_SIZE,
                        "same_state_only": True,
                        "debatched_raw_axes": "state,noise,arm,time,position,coordinate",
                    },
                },
            )
            _deadline(started, f"state {state_index} B2")
            engineering["completed"] = completed.tolist()
            engineering["prefix_cache_digests"] = prefix_cache_digests
            engineering["last_completed"] = {"state": state_index, "noise": BATCH_SIZE - 1}
            _atomic_json(output / "engineering.json", engineering)
        _restore_fp(modules, fp, torch)
        if helper._state_subset_digest(policy, expert_state_names, torch) != bypass_digest:
