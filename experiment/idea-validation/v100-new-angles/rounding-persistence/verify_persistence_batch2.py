"""Independent CPU replay and receipt verification for the B2 persistence screen.
...
SCREEN_SCHEMA = "rounding-persistence-batch2-screen-v1"
VERIFICATION_SCHEMA = "rounding-persistence-batch2-verification-v1"
...
EXPECTED_RUNNER_SHA256 = "01059efb28661486931b935ad4a3a565456c043bc76985873a571bbf8c09c700"
BATCH_MODE_RECEIPT = {
    "batch_size": 2,
    "conditions_per_forward": 2,
    "same_state_only": True,
    "raw_axes": "debatched back to state,noise,arm,time,position,coordinate",
}
RAW_BATCH_MODE_RECEIPT = {
    "batch_size": 2,
    "same_state_only": True,
    "debatched_raw_axes": "state,noise,arm,time,position,coordinate",
}
COMMON_HASH_FORMULA = "sha256(contiguous float32 bytes of each exact per-sample x_t captured immediately before each denoise_step)"
...
def _common_hash_check(data: Mapping[str, Any], np: Any) -> Dict[str, Any]:
    mismatch, mismatch_count, malformed_count = [], 0, 0
    for state in range(6):
    ...
    return {"common_x_sha256_matches_fp_trajectory": mismatch_count == 0, "common_x_hash_count": 360, "common_x_hash_mismatch_count": mismatch_count, "common_x_hash_malformed_count": malformed_count, "common_x_hash_per_sample_batch_order": "common_x_sha256[state,noise,draw,step] is one hash for each B2 noise member", "common_x_hash_mismatch_examples": mismatch, "hash_formula": "sha256(contiguous float32 bytes of raw_x[state,noise,FP,step])"}
...
def _incomplete_verdict(
    input_dir: Path, output_dir: Path, engineering_path: Path, engineering: Mapping[str, Any], allocation: Mapping[str, Any]
) -> Dict[str, Any]:
    """Classify a producer artifact without loading or interpreting partial raw arrays."""
    producer_status = engineering.get("status")
    budget_statuses = {"running", "inconclusive", "inconclusive_budget"}
    decision = "inconclusive_budget" if producer_status in budget_statuses else "implementation_inconclusive"
    raw_path = input_dir / "raw_persistence.npz"
    return {
        "schema": VERIFICATION_SCHEMA,
        "allocation": dict(allocation),
        "input_job_dir": str(input_dir),
        "output_job_dir": str(output_dir),
        "engineering_sha256": sha(engineering_path),
        "checks": {
            "engineering_complete": False,
            "raw_archive_present": raw_path.is_file(),
            "b2_batch_mode_receipt": engineering.get("batch_mode") == BATCH_MODE_RECEIPT,
        },
        "receipts": {
            "engineering_status": producer_status,
            "completed": engineering.get("completed"),
            "batch_mode": engineering.get("batch_mode"),
        },
        "numeric_decision": decision,
        "decision": decision,
        "independence_boundary": "Partial GPU arrays are never interpreted as science; this status records only the producer allocation/output boundary.",
        "scope": "B2 GPU artifact is incomplete; no scientific metrics or partial-array replay were computed.",
    }
...
def _verify_artifact(args: argparse.Namespace, allocation: Mapping[str, Any], np: Any) -> Dict[str, Any]:
    ...
    if engineering.get("status") != "complete" or engineering.get("completed") != expected_completed:
        return _incomplete_verdict(input_dir, output_dir, engineering_path, engineering, allocation)
    data, raw_path = _load_raw(input_dir, np)
    deadline("raw load")
    summary = _read_json(summary_path, "screen summary")
    checks: Dict[str, bool] = {}
    receipts: Dict[str, Any] = {}
    checks["producer_source_frozen_pin"] = sha(input_dir / "persistence_batch2_screen.py") == EXPECTED_RUNNER_SHA256
    ...
    checks["raw_metadata_receipt_binding"] = metadata.get("manifest_path") == info.get("path") and metadata.get("manifest_sha256") == info.get("sha256") and metadata.get("identity_sha256") == info.get("identity_sha256") and metadata.get("schedule_definition") == "frozen[d,t]=d; cyclic[d,t]=(d+t)%3" and metadata.get("physical_readout") == "first 7 action coordinates; common path retains padded 32D velocity" and metadata.get("common_x_sha256_formula") == COMMON_HASH_FORMULA and metadata.get("batch_mode") == RAW_BATCH_MODE_RECEIPT
    batch_mode = engineering.get("batch_mode")
    checks["engineering_b2_batch_mode"] = batch_mode == BATCH_MODE_RECEIPT
    params = engineering.get("parameters", {})
    expected_params = {"arms": list(ARM_NAMES), "draw_seeds": list(DRAW_SEEDS), "noise_seeds": list(NOISE_SEEDS), "noise_shape": [1, 50, 32], "noise_batch_shape": [2, 50, 32], "timestep_batch_shape": [2], "timestep_source": "LeRobot v0.4.4 sample_actions time_tensor.expand(bsize)", "raw_x_shape": [6, 2, 8, 11, 50, 32], "raw_v_shape": [6, 2, 8, 10, 50, 32], "action_shape": [6, 2, 8, 50, 7], "common_q_v_shape": [6, 2, 3, 10, 50, 32], "common_x_sha256_shape": [6, 2, 3, 10], "actual_times_shape": [6, 2, 8, 10], "expert_linear_count": 112, "dt": -0.1, "bits": 4, "qmax": 7}
    checks["engineering_frozen_parameters"] = all(params.get(k) == v for k, v in expected_params.items())
    checks["engineering_parameters_batch_mode"] = params.get("batch_mode") == BATCH_MODE_RECEIPT
    checks["engineering_manual_fp_gate"] = engineering.get("manual_fp_gate", {}).get("allclose_atol_1e-6_rtol_0") is True
    checks["engineering_b2_manual_shape"] = engineering.get("manual_fp_gate", {}).get("official_shape") == [2, 50, 7]
    ...
    checks.update({"common_x_sha256_matches_fp_trajectory": common["common_x_sha256_matches_fp_trajectory"],
                   **{key: value for key, value in schedule_time.items() if isinstance(value, bool)}})
