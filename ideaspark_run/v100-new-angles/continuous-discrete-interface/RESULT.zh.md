...
因此不把source-available当成可用基线，不为这个对比下载另一份大checkpoint或切换训练目标。相关代码位置、公开配置与资产边界保留于 [资产审查](../../../tmp/idea-screening-20260912/vla_wam_asset_map.md)；原始source在 `reproduction/openvla-oft-vla-eval-libero/source/openvla-oft`。该候选停在pipeline结构审查，不做完整实验。
