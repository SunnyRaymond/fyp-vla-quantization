...
贡献审查补充：**generic method novelty no-go**。更直接的De-biasing Diffusion prior已提出stochastic weights减少跨denoising-step误差相关性；本案只保留matched-marginal SmolVLA诊断signal，见[novelty边界](NOVELTY_BOUNDARY.zh.md)。不改变下面原始数值结论。
