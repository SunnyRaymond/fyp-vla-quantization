#!/usr/bin/env bash
# B2 execution-efficiency repair for the matched temporal-coupling screen.
# Each state owns one batch containing the two fixed noises; raw output is
# de-batched back to the original persistence schema.  This script is only a
# launcher: model loading, hashing, input processing and inference run inside
# the real CCDS SLURM allocation checked below.
...
#SBATCH --job-name=rounding_persistence_b2
#SBATCH --output=artifacts/rounding_persistence_b2_%j.log
...
PROTOCOL_COPY="${ROUNDING_PERSISTENCE_PROTOCOL_COPY:-${CONTROL}/persistence_batch2_protocol.zh.md}"
RUNNER="${ROUNDING_PERSISTENCE_RUNNER:-${CONTROL}/persistence_batch2_screen.py}"
...
mkdir -p "${OUT}"
cp "${RUNNER}" "${OUT}/persistence_batch2_screen.py"
...
cp "${PROTOCOL_COPY}" "${OUT}/persistence_batch2_protocol.zh.md"
...
timeout --signal=TERM --kill-after=15s 9m \
  "${PYTHON}" "${OUT}/persistence_batch2_screen.py" \
  --input-manifest "${MANIFEST}" \
  --model-path "${MODEL}" \
  --checkpoint "${CHECKPOINT}" \
  --vlm-path "${VLM}" \
  --flow-helper "${OUT}/flow_screen.py" \
  --protocol-copy "${OUT}/persistence_batch2_protocol.zh.md" \
  --output "${OUT}" \
  --max-seconds 540 \
  "${EXTRA_ARGS[@]}" \
