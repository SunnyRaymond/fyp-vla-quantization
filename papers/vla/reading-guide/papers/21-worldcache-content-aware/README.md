...
**同名边界。** 这篇不是 [WorldCache: Accelerating World Models for Free via Heterogeneous Token Caching](../22-worldcache-heterogeneous-token-caching/README.md)。本篇用 `CFC + SWD + OFA + ATS` 决定 deep blocks 何时缓存、如何补偿；`#22` 用 token-level curvature 把 outputs 分成 stable/linear/chaotic。作者、arXiv、venue、model 和 cache granularity 均不同。
