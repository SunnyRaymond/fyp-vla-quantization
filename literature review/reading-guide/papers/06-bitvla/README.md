...
[LLaVA](../06b-llava/README.md) 是 multimodal training recipe 的来源，不是 BitVLA 的 robot policy backbone：
...
BitVLA 自身并不走 bitsandbytes；native W1.58A8 inference 用 BitBLAS custom kernel。BitVLA citation `[9]` 对应 [LLM.int8()](../06d-llm-int8-bitsandbytes/README.md)：该 algorithm 用 vector-wise INT8 处理 regular features，并把少量 outlier feature dimensions 留给 FP16 matmul。因此 `INT8` baseline 也不是所有 operation 都是 pure INT8。
...
BitVLA 直接采用 [OpenVLA-OFT](../06c-openvla-oft/README.md) Table III 的 ALOHA input shape 与 baseline numbers 来做 Fig. 6 comparison。`341.1 Hz` 是 `K/latency` 式的 amortized action-generation throughput，不等于每秒 341 次重新看三张图。
...
7. 补读 [LLaVA](../06b-llava/README.md)、[OpenVLA-OFT](../06c-openvla-oft/README.md)、[LLM.int8/bitsandbytes](../06d-llm-int8-bitsandbytes/README.md) 与 [Apprentice](../06a-apprentice-quantization-distillation/README.md)，把 inherited components 与 BitVLA contribution 分开。
