...
> **Suggested prerequisites:** [Flow Matching](../17-flow-matching/README.md), `Diffusion Transformer`, `EMA teacher`, `REPA`, `SRA`, `FID/FVD/FAD`, and basic `SIMPLER` evaluation vocabulary.  
...
Read [Flow Matching](../17-flow-matching/README.md) first if equations (1)-(2) feel unfamiliar. Read [REPA / iREPA → VLA representation alignment](../19-repa-irepa-vla-alignment/README.md) if the external-alignment baseline or the frozen-teacher boundary is unclear.
...
1. Re-derive equations (1)-(2) from [Flow Matching](../17-flow-matching/README.md).
