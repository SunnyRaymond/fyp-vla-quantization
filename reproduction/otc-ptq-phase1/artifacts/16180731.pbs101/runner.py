...
def _restore_snapshot(env: Any, snapshot: Mapping[str, Any]) -> dict[str, np.ndarray]:
    ...
    try:
        ...
        _restore_sim_fields(sim, snapshot["sim_fields"])
        _restore_env_mutable(env, snapshot["env_mutable"], restore_observables=False, restore_obs_cache=False)
        core = _env_core(env)
        ...
        if hasattr(core, "_get_observations"):
            ...
            raise RunnerError("LIBERO environment has no observation getter.")
        max_error = _obs_max_abs_error(snapshot["obs"], restored_obs)
        if max_error > 1.0e-4:
            raise RunnerError(f"Observation mismatch after restore: max_abs_error={max_error}")
        # Force-update changes observable timers and sampled flags. Restore the
        # complete captured bookkeeping a second time, then verify the live
        # references remain the same objects and observations are unchanged.
...
def _probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    snapshot["contacts"] = _filter_task_contacts(snapshot["contacts"])
    import torch
...
def _collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    while t < limit:
        ...
        if not pending:
            ...
            snapshot["contacts"] = _filter_task_contacts(snapshot["contacts"])
            before_contacts = snapshot["contacts"]
    ...
    def priority(record: Mapping[str, Any]) -> tuple[int, int, int]:
        ...
        active = bool(record.get("contacts", {}).get("count", 0))
