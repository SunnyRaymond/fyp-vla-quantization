...
def _score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for state_index, snapshot in enumerate(states):
        ...
        if collected_qpos_error > 1e-10 or reference['done'] != snapshot['terminal_after_first_action']:
            raise RunnerError(f"Collected physics/terminal not reproduced: qpos={collected_qpos_error}")
        validation = {
