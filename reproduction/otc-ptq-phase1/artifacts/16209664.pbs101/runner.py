...
def _restore_buffer(value: Any, saved: Mapping[str, Any], label: str) -> None:
    ...
    # DeltaBuffer.push replaces arrays: dummy integer actions and policy float
    # actions legitimately have different dtypes. Restore the saved history's
    # dtype, rather than truncating it to the freshly initialized buffer dtype.
    for name, leaf in saved.get("attrs", {}).items():
        if not hasattr(value, name):
            raise RunnerError(f"Buffer attribute missing: {label}.{name}")
        current = getattr(value, name)
        if isinstance(current, np.ndarray) and isinstance(leaf, np.ndarray) and current.shape != leaf.shape:
            raise RunnerError(f"Buffer shape changed: {label}.{name}")
        setattr(value, name, _copy_state_value(leaf))
...
def _capture_env_mutable(env: Any) -> dict[str, Any]:
    ...
    def capture_visual_flags(collection: Any, label: str) -> dict[str, Any]:
        if not isinstance(collection, Mapping):
            raise RunnerError(f"Required object registry is not a mapping: {label}")
        result: dict[str, Any] = {}
        for name, obj in collection.items():
            properties = getattr(obj, "object_properties", None)
            if properties is None:
                continue
            if not isinstance(properties, Mapping):
                raise RunnerError(f"Object properties are not a mapping: {label}.{name}")
            if "vis_site_names" in properties:
                result[str(name)] = _copy_state_value(properties["vis_site_names"])
        return result

    objects = getattr(core, "objects_dict", None)
    fixtures = getattr(core, "fixtures_dict", None)
    object_states = getattr(core, "object_states_dict", None)
    tracking_states = getattr(core, "tracking_object_states_change", None)
    if not isinstance(objects, Mapping) or not isinstance(fixtures, Mapping) or not isinstance(object_states, Mapping):
        raise RunnerError("LIBERO object/fixture/object-state registries are unavailable")
    if tracking_states is None:
        raise RunnerError("LIBERO tracking_object_states_change registry is unavailable")
    tracking_names = []
    for index, state_obj in enumerate(tracking_states):
        name = getattr(state_obj, "object_name", None)
        if name is None:
            raise RunnerError(f"Tracked object state has no object_name: index {index}")
        tracking_names.append(str(name))
    result: dict[str, Any] = {
        "schema": 3,
        ...
        "observables": {},
        "registry_keys": {
            "objects": [str(name) for name in objects.keys()],
            "fixtures": [str(name) for name in fixtures.keys()],
            "object_states": [str(name) for name in object_states.keys()],
            "tracking_object_states": tracking_names,
        },
        "visual_flags": {
            "objects": capture_visual_flags(objects, "env.objects_dict"),
            "fixtures": capture_visual_flags(fixtures, "env.fixtures_dict"),
        },
    }
...
def _restore_env_mutable(
    ...
    *,
    ...
    def restore_visual_flags(collection: Any, saved: Any, label: str) -> None:
        if not isinstance(collection, Mapping) or not isinstance(saved, Mapping):
            raise RunnerError(f"Visual state registry is unavailable: {label}")
        failures: list[str] = []
        for name, expected in saved.items():
            if name not in collection:
                failures.append(f"{label}.{name}: object missing")
                continue
            obj = collection[name]
            properties = getattr(obj, "object_properties", None)
            if not isinstance(properties, Mapping) or "vis_site_names" not in properties:
                failures.append(f"{label}.{name}: vis_site_names missing")
                continue
            try:
                current = properties["vis_site_names"]
                restored = _restore_like(current, expected)
                # Keep the live dict used by LIBERO object definitions in place.
                if isinstance(current, Mapping) and isinstance(restored, Mapping):
                    current.clear()
                    current.update(restored)
                else:
                    properties["vis_site_names"] = restored
            except Exception as exc:
                failures.append(f"{label}.{name}: {exc}")
        if failures:
            raise RunnerError("Visual object state restore failed: " + "; ".join(failures))

    registries = state.get("registry_keys")
    if not isinstance(registries, Mapping):
        raise RunnerError("Snapshot lacks LIBERO object registry keys")
    for name, current in (
        ("objects", getattr(core, "objects_dict", None)),
        ("fixtures", getattr(core, "fixtures_dict", None)),
        ("object_states", getattr(core, "object_states_dict", None)),
    ):
        expected = [str(key) for key in registries.get(name, [])]
        if not isinstance(current, Mapping) or [str(key) for key in current.keys()] != expected:
            raise RunnerError(f"LIBERO {name} registry changed across state restore")
    current_tracking = getattr(core, "tracking_object_states_change", None)
    actual_tracking = [str(getattr(item, "object_name", "")) for item in (current_tracking or [])]
    if actual_tracking != [str(key) for key in registries.get("tracking_object_states", [])]:
        raise RunnerError("LIBERO tracking_object_states_change registry changed across state restore")
    visual_flags = state.get("visual_flags")
    if not isinstance(visual_flags, Mapping):
        raise RunnerError("Snapshot lacks LIBERO visual object state")
    restore_visual_flags(getattr(core, "objects_dict", None), visual_flags.get("objects", {}), "env.objects_dict")
    restore_visual_flags(getattr(core, "fixtures_dict", None), visual_flags.get("fixtures", {}), "env.fixtures_dict")
    robots = list(getattr(core, "robots", []) or [])
...
def _state_tree_max_abs(expected: Any, actual: Any, label: str = "state") -> float:
    ...
    try:
        error = _numeric_max_abs(expected, actual)
        if error > 1e-7:
            LOGGER.error("State leaf mismatch: %s max_abs=%g expected_dtype=%s actual_dtype=%s",
                         label, error, np.asarray(expected).dtype, np.asarray(actual).dtype)
        return error
...
def _restore_snapshot(env: Any, snapshot: Mapping[str, Any]) -> dict[str, np.ndarray]:
    ...
    try:
        ...
        # mj_step ends after integration: saved contacts describe its previous
        # position stage. mj_forward derives contacts at the new qpos instead.
        # robosuite step() itself calls forward before control, so this derived
        # cache is audited, not treated as an integration-state invariant.
        restored_contacts = _capture_contacts(env)
        if not snapshot['contacts'].get('available') or not restored_contacts.get('available'):
            raise RunnerError('Contact diagnostics unavailable after restore')
        contact_cache_equal = snapshot['contacts'] == restored_contacts
        ...
        if isinstance(snapshot, MutableMapping):
            snapshot["restore_diagnostics"] = {
                ...
                "contacts_exact": contact_cache_equal,
                "saved_raw_contact_points": snapshot['contacts']['count'],
                "forward_raw_contact_points": restored_contacts['count'],
                "contact_cache_contract": "derived mj_forward cache may differ; original next-step contact/physics equality is validated separately",
...
_REUSE_RUNTIME = False


def _make_runtime(args: argparse.Namespace) -> Runtime:
    global _RUNTIME
    if _REUSE_RUNTIME and _RUNTIME is not None:
        if _RUNTIME.task_id != args.task_id:
            raise RunnerError('A batch cannot change task identity')
        return _RUNTIME
    ...
    print("OTC runtime: constructing model", flush=True)
    model = instantiate(cfg.model, model_dtype=torch.bfloat16, device="cuda")
    print("OTC runtime: loading pinned checkpoint", flush=True)
    model.load_checkpoint(str(checkpoint))
    model = model.to("cuda").eval()
    print("OTC runtime: checkpoint loaded", flush=True)
...
def _start_episode(runtime: Runtime, episode: int, *, wait_steps: int = 0) -> tuple[dict[str, np.ndarray], Any]:
    from fastwam.utils.pytorch_utils import set_global_seed
    if runtime.cfg.get('seed') is not None:
        set_global_seed(int(runtime.cfg.seed), get_worker_init_fn=False)
...
def _filter_task_contacts(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for pair in raw.get("pairs", []):
        ...
        if (lhs_robot and rhs_robot) or persistent:
            ...
            continue
        kept.append([lhs, rhs])
    # MuJoCo can expose several manifold points for one contacting geom pair.
    # Contact events use unique unordered pairs, not fluctuating point counts.
    unique_pairs = {tuple(sorted(pair, key=str)) for pair in kept}
    kept = [list(pair) for pair in sorted(unique_pairs, key=str)]
    return {
        "available": True,
        ...
        "predicate": "unique non-support candidate geom pairs; includes object-object",
    }
...
def _collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    while t < limit:
        ...
        if not pending:
            ...
            # Continue the untouched trajectory: first action already executed.
            # Rewinding here would also rewind policy RNG consumption.
            obs = _copy_obs(next_obs)
            pending = action_chunk[1:DEFAULT_REPLAN_STEPS].tolist()
...
def _score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _start_episode(runtime, int(args.episode), wait_steps=DEFAULT_NUM_STEPS_WAIT)
    ...
    for state_index, snapshot in enumerate(states):
        ...
        collected_qpos_error = float(np.max(np.abs(reference['next_physics']['qpos'] - snapshot['reference_next_physics']['qpos'])))
        collected_qvel_error = float(np.max(np.abs(reference['next_physics']['qvel'] - snapshot['reference_next_physics']['qvel'])))
        collected_contact_error = int(reference['next_contacts'] != snapshot['reference_next_contacts'])
        ...
        if collected_qpos_error > 1e-10 or collected_qvel_error > 1e-10 or collected_contact_error or reference['done'] != snapshot['terminal_after_first_action']:
            raise RunnerError(f"Collected physics/contact/terminal not reproduced: qpos={collected_qpos_error}, qvel={collected_qvel_error}, contact={collected_contact_error}")
        validation = {
            "collected_reference_action_max_abs": collected_action_error,
            "collected_reference_next_obs_l1": collected_pixel_error,
            "collected_reference_qpos_max_abs": collected_qpos_error,
            "collected_reference_qvel_max_abs": collected_qvel_error,
            "collected_reference_contact_disagreement": collected_contact_error,
            "reference_repeat_qpos_max_abs": float(np.max(np.abs(reference['next_physics']['qpos'] - repeat['next_physics']['qpos']))),
            "reference_identity_qpos_max_abs": float(np.max(np.abs(reference['next_physics']['qpos'] - identity['next_physics']['qpos']))),
        ...
        if validation['reference_repeat_qpos_max_abs'] > 1e-10 or validation['reference_identity_qpos_max_abs'] > 1e-10 or not reference['done'] == repeat['done'] == identity['done']:
            raise RunnerError(f"Reference/identity physics or terminal mismatch: {validation}")
        print(f"OTC score: state {state_index + 1}/{len(states)} replay passed", flush=True)
        for site in runtime.sites:
            try:
                ...
                ref_objects = reference['next_physics'].get('object_body_xpos', {})
                quant_objects = quant['next_physics'].get('object_body_xpos', {})
                if set(ref_objects) != set(quant_objects):
                    raise RunnerError('Task-body registry changed between branches')
                object_deltas = {name: float(np.linalg.norm(quant_objects[name] - pos))
                                 for name, pos in ref_objects.items()}
                row = {
                    "status": "ok",
                    ...
                    "object_position_delta_m": object_deltas,
                    "max_object_position_delta_m": max(object_deltas.values()) if object_deltas else None,
                    "task_physics_scope": reference['next_physics'].get('task_physics_scope'),
                    ...
                    "restore_diagnostics": snapshot.get('restore_diagnostics'),
                }
            except RunnerError:
                raise
            except Exception as exc:
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--episodes", type=int, nargs='+', default=None,
                        help='Reuse one loaded model for these episodes of the same task')
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    episodes = args.episodes if args.episodes is not None else [args.episode]
    if len(set(episodes)) != len(episodes) or any(e not in range(4) for e in episodes):
        raise RunnerError('Batch episodes must be unique initial-state indices 0..3')
    if args.mode == 'probe' and len(episodes) != 1:
        raise RunnerError('Probe remains a single-episode gate')
    _REUSE_RUNTIME = len(episodes) > 1
    try:
        for episode in episodes:
            args.episode = episode
            print(f'OTC batch: {args.mode} task={args.task_id} episode={episode}', flush=True)
            if args.mode == "probe":
                _probe(args)
