...
def main():
    ...
    client, user = connect()
    try:
        base = '/tc1home/UG/' + user + '/cem_update_ccds'
        ...
        if args.action == 'inventory':
            command = '\n'.join(['hostname', 'pwd', 'module load slurm',
                ...
                'sacctmgr -nP show qos normal format=name,MaxTRESPU,MaxJobsPU,MaxWall',
                'sinfo -h -o "%P %a %l %D %t %G"',
                'squeue -u '+shlex.quote(user)+' -o "%.16i %.20j %.8T %.10M %R"',
                'ls -d "$HOME"/cem_update_ccds "$HOME"/dino* "$HOME"/.conda/envs/* 2>/dev/null || true'])
