...
当前 flow 资产记录了真实 raw keys `observation.images.image` 与 `observation.images.image2`，saved preprocessor 后分别成为 `observation.images.camera1` 与 `observation.images.camera2`；checkpoint 还声明 camera3，但 dataset 没有它，且 `empty_cameras=0`。见本地 [STATE_INTERFACE_AUDIT](../../../experiment/idea-validation/v100-new-angles/flow-geometry-drift/STATE_INTERFACE_AUDIT.zh.md) 与 [flow geometry result](../../../experiment/idea-validation/v100-new-angles/flow-geometry-drift/RESULT.zh.md)。因此现有 camera2 是 wrist view，不能被称为与 camera1 的 duplicate/redundant view。
...
- 本地 [Flow Geometry under PTQ result](../../../experiment/idea-validation/v100-new-angles/flow-geometry-drift/RESULT.zh.md)：已有两个真实 camera、FP/backbone-W4/expert-W4 的 drift screen，但没有 mask factor；因此它是接口资产与 metric 邻域，不能当作 redundancy 证据。
