#!/usr/bin/env python3
"""Run the frozen paired official-CEM versus iteration-cache PushT transfer.
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("--control-root", type=Path, default=Path(__file__).resolve().parent)
    parser.add_argument(
        "--asset-root",
        type=Path,
        default=None,
    ...
    parser.add_argument(
        "--freeze",
        type=Path,
        default=Path(__file__).with_name("ITERATION_CACHE_FREEZE.json"),
    )
...
class TraceCEMPlanner:
    """Official CEM loop with original rollout or cross-iteration cache."""

    def __init__(self, official_cls: type, *, cache_mode: str, trace: TraceRun, **kwargs: Any) -> None:
        self._planner = official_cls(**kwargs)
        self._cache_mode = cache_mode
        self._plan_section_latency_ms: float | None = None
        self._cem_loop_latency_ms: float | None = None
        self._cache_build_latency_ms = 0.0
        self._cache_build_count = 0

    @property
    def plan_section_latency_ms(self) -> float:
        if self._plan_section_latency_ms is None:
            raise RuntimeError("plan_section latency is not available before plan()")
        return self._plan_section_latency_ms

    @property
    def cache_build_latency_ms(self) -> float:
        return self._cache_build_latency_ms

    @property
    def cache_build_count(self) -> int:
        return self._cache_build_count

    @property
    def cem_loop_latency_ms(self) -> float:
        if self._cem_loop_latency_ms is None:
            raise RuntimeError("cem_loop latency is not available before plan()")
        return self._cem_loop_latency_ms
    ...
    def plan(self, obs_0: dict[str, Any], obs_g: dict[str, Any], actions: Any = None):
        ...
        from cache_core import encode_and_repeat, rollout_from_encoded_obs
        from utils import move_to_device

        planner = self._planner
        self._plan_section_latency_ms = None
        self._cem_loop_latency_ms = None
        self._cache_build_latency_ms = 0.0
        self._cache_build_count = 0
        ...
        # Secondary timing boundary starts after both observation transforms
        # and includes goal encoding, cache setup, the full CEM loop and logs.
        torch.cuda.synchronize()
        plan_section_started = time.perf_counter()
        z_obs_g = planner.wm.encode_obs(trans_obs_g)

        if self._cache_mode not in {"baseline", "iteration_cache"}:
        ...
        cached_prefixes: list[dict[str, torch.Tensor] | None] = [None] * n_evals

        torch.cuda.synchronize()
        cem_loop_started = time.perf_counter()
            ...
            for traj in range(n_evals):
                ...
                with torch.no_grad():
                    if self._cache_mode == "baseline":
                        ...
                        if cached_prefixes[traj] is None:
                            cache_started = time.perf_counter()
                            cached_prefixes[traj] = {
                                key: value.detach()
                                for key, value in encode_and_repeat(
                                    planner.wm,
                                    cur_trans_obs_0,
                            ...
                            torch.cuda.synchronize()
                            self._cache_build_latency_ms += (
                                time.perf_counter() - cache_started
                            ) * 1000.0
                            self._cache_build_count += 1
                        i_z_obses, _ = rollout_from_encoded_obs(
                            planner.wm, cached_prefixes[traj], action
                        )
        ...
        torch.cuda.synchronize()
        self._cem_loop_latency_ms = (
            time.perf_counter() - cem_loop_started
        ) * 1000.0
        self._plan_section_latency_ms = (
            time.perf_counter() - plan_section_started
        ) * 1000.0
...
def _timed_call(
    ...
    result = {
        "full_planner_latency_ms": full_latency_ms,
        "latency_ms": full_latency_ms,
        "plan_section_latency_ms": planner.plan_section_latency_ms,
        "cem_loop_latency_ms": planner.cem_loop_latency_ms,
        "cache_build_latency_ms": planner.cache_build_latency_ms,
        "cache_setup_latency_ms": planner.cache_build_latency_ms,
        "cache_build_count": planner.cache_build_count,
...
def main() -> int:
    ...
    source = root / "source"
    idea_root = args.control_root.resolve()
    ...
    for obs_index, (observation_id, obs_0, obs_g) in enumerate(observations):
        for warmup in range(warmups):
            ...
            order = (
                ["baseline", "iteration_cache"]
                if (obs_index + warmup) % 2 == 0
                else ["iteration_cache", "baseline"]
            )
            for path in order:
                trace = TraceRun()
    ...
    summary = {
        "schema": "dino-wm-shared-prefix-cache.pusht-iteration-cache-results",
        "schema_version": 1,
        ...
        "plan_section_boundary": timing["plan_section_boundary"],
    ...
    (output / "PUSHT_ITERATION_CACHE_COMPLETE").touch()
