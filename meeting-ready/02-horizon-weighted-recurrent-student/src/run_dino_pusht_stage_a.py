...
def _load_official(
    ...
    source = root / "source"
    # ``cache_core`` is bundled beside the runners.  The continuation bundle
    # must not depend on the old idea-validation tree for teacher rollouts.
    local_helpers = Path(__file__).resolve().parent
    if str(local_helpers) not in sys.path:
        sys.path.insert(0, str(local_helpers))
